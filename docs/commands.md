...
Every command is a dry run unless explicitly applied, prints a
human-readable report by default, and emits a machine-readable envelope
with `--json` (one document) or `--json-stream` (NDJSON: progress
events while the command runs, the result as the last line). Exit codes
everywhere: `0` clean, `1` findings needing attention, `2` errors.
...
from the card, so a transfer error cannot slip through.
...
Every bucket carries a severity — alarm, attention, expected or safe —
so scripts never re-derive what a bucket means.
...
Every apply is journaled to `~/.chronocatalog/journals/` before the
first change. `chronocatalog history` lists every recorded run with its
originating command and status (pending, partial, complete, undone),
...
an interrupted apply is exactly the journal's partial case.
